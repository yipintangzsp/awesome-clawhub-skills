...
async function fetchApiKey() {
  ...
  if (token && contextFile) {
    try {
      ...
      if (apiUrl && poolId) {
        const url = `${apiUrl}/api/internal/secrets/nano-banana-one-shop?poolId=${poolId}`;
...
// Desktop bundles sharp and exposes it through NODE_PATH for skill scripts.
...
try {
  ...
  console.error(
    `ERROR: sharp is required but was not bundled into this runtime (${reason}).`,
  );
  process.exit(1);
}
